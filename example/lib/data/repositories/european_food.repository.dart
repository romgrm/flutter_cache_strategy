...
class EuropeanFoodRepository {
  ...
  Future<List<MealEntity>> getEuropeanFood() async {
    ...
    final List<MealDto.MealDto>? frenchFood = await _package.execute(
        keyCache: "frenchFood", boxeName: "BOXE ENCRYPTED", serializer: (p0) => MealDto.MealDto.fromData(p0), async: provider.getFrenchFood(), strategy: AsyncOrCacheStrategy(), isEncrypted: false);
