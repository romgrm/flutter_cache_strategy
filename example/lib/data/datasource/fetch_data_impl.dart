...
class FetchDataImpl implements FetchData {
  ...
  Future<List<MealDto>?> getFrenchFood() async {
    final json = await provider.get(
        path: "https://www.themealdb.com/api/json/v1/1/filter.php?a=French");

    return List<MealDto>.from(
        json['meals'].map<MealDto>((meal) => MealDto.fromJson(meal)));
  }

  @override
  Future<List<MealDto>?> getItalianFood() async {
    final json = await provider.get(
        path: "https://www.themealdb.com/api/json/v1/1/filter.php?a=Italian");
  ...
  Future<List<MealDto>?> getIndianFood() async {
    final json = await provider.get(
        path: "https://www.themealdb.com/api/json/v1/1/filter.php?a=Indian");
